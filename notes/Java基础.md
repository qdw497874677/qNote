...
- 依赖（Dependency）：在一个类中临时性的使用另外一个类。例如方法参数、方法内创建对象、方法内调用其他类静态资源
  - ![image-20210417141414341](Java基础.assets/image-20210417141414341.png)
- 关联（Association）：两个独立的对象实例存在固定的对应关系。比如对象中有另外一个对象作为成员变量。
  - 单向和双向
  - ![image-20210417141649854](Java基础.assets/image-20210417141649854.png)
- 聚合（Aggregation）：是关联的一种，耦合程度高于关联。
  - ![image-20210417151119955](Java基础.assets/image-20210417151119955.png)
- 组合（Composition）：相比于聚合，耦合度更强。
  - ![image-20210417151219902](Java基础.assets/image-20210417151219902.png)
- 继承（Generalization）：
  - ![image-20210417152104547](Java基础.assets/image-20210417152104547.png)
- 实现（Implementation）：
  - ![image-20210417152130810](Java基础.assets/image-20210417152130810.png)
...
- 修饰代码块：静态代码块，在类被初次加载时执行静态代码块（在构造方法前），只有执行一次。对于定义在他后面的静态变量可以赋值（不能对非静态的变量赋值），但是不能访问。
  - 和非静态代码块（构造代码块）的相同与不同：
    ...
    - 不同点：静态代码块在非静态代码块之前执行。静态代码块只在第一次类初始化执行一次（特例：Class.forName("ClassDemo")创建Class对象的时候也会执行。）；非静态代码块是在对象每初始化一次就执行一次。非静态代码块可在普通方法中定义；静态代码块不行。
...
非静态代码块是给所有对象进行统一初始化，而构造方法是给对应的对象初始化，因为构造方法可以有多个。
...
finalize()是Object类中的成员方法。当对象被GC宣布死亡时，先调用finalize()，只执行一次，但是具有不确定性，只保证调用，不保证全部完成。自救方法：重写finalize()，让对象重新联系上GCroot链。
...
> 在同步块中，一个线程调用wait()方法后，当前调用线程**将进入等待状态**或者调用带超时时间地**进入超时等待状态**，进入对象锁的等待队列中，同时**释放持有的锁**，直到被唤醒，即其他线程调用此对象锁的notify()或notifyAll()，后重新竞争锁。wait()只能在同步代码中调用，也就是说必须先获得锁，调用后会释放锁，进入等待状态，让出CPU。
>
> 与sleep()的区别
>
...
如果想让逻辑上相等的对象在哈希表中表示相等，就需要重写eqauls和hashcode
...
2. 当通过new 创建字符串对象时。new String("abc")。在堆中创建一个String对象，如果在常量池如果没有这个字符串就创建字符串，让String对象中的数据引用指向常量池中的。如常量池中有对应字符串就直接让String对象中的引用指向。**无论常量池中是否有，最终都要在堆中创建一个String对象且数据变量都指向常量池中的字符串**
...
迭代器中的引用的数据和集合中是同一份。在迭代器中维护一个自己的**操作计数**，当remove时自己的计数和集合的计数同步，如果在next或者remove时发现两个计数不一致，说明集合在迭代过程中改动过，就会抛出并发修改异常。
...
反射可以在运行时获取Class类，通过Class类可以获取类的属性和方法，以及创建实例，调用方法。
