...
- 修饰成员变量和成员方法：被static修饰的成员属于类，可以通过类名调用。静态方法只能访问和调用静态的。在方法区（元空间）中。
- 成员变量：静态变量，通过类访问。
- 代码块：静态代码块，在类被初次加载时，执行静态代码块，只有执行一次。对于定义在他后面的静态变量可以赋值（不能对非静态的变量赋值），但是不能访问。
  - 和非静态代码块（构造代码块）的相同与不同：
    - 相同点：都是在JVM加载类时且在构造方法之前执行，在类中都可以定义多个，定义多个时按照顺序执行，一般在代码块中对一些static变量进行赋值。
...
同步非阻塞，通过多线程的方式实现非阻塞。线程过多会导致cpu资源消耗大。

异步阻塞，比如数据库从库的同步，一般是通过异步阻塞。
...
客户端与服务器端通信，**客户端要创建一个Socket实例**，操作系统为这个Socket实例分配一个没有被使用过的本地端口号，并创建一个包含本地和远程地址和端口号的套接字数据结构。这个数据结构一直保存在系统中，直到这个连接关闭。在穿件Socket市里的构造函数返回正确之前，要进行TCP的三次握手，握手完，Socket实例对象将创建完成，否则抛出IOException错误。
...
NIO，就是阻塞IO，一个线程去读取网络数据，但是无法预知对方已经发送数据，因此在收到数据之前，只能进入阻塞等待。可以通过多个线程同时等待，但是过多的线程导致过的的上下文切换，浪费cpu资源，而且每个线程也会占用内存空间。

## NIO

NIO。在获取数据时，如果没有可以立刻返回。通过不断轮询的方式获取数据。
...
IO多路复用要和NIO一起使用。NIO表示可以立刻返回，IO多路复用是操作系统提供的一种通知机制。
...
反射是Java被视为动态语言的关键。加载完类之后，在堆内存中生成一个Class类型的对象，这个对象包含了完整的类的结构信息。通过这个Class类得到类的信息，就是反射。
