...
要想将对象序列化和反序列化，对象的类需要实现serializable接口。使用transient修饰的变量的值不会被序列化。
...
1. 当使用字面值（String s = "hello";）创建一个字符串时，JVM会先检查常量池中是否存在。如果存在，那么引用指向的就是常量池中存在的字符串对象；如果常量池中不存在，就会实例化字符串对象放在常量池中，然后引用指向的这个常量池中的字符串对象。也就是说无论常量池中是否有，**引用都指向的是常量池中的对象**。
2. 当通过new 创建字符串对象时。new String("abc")。在堆中创建一个字符串对象，在常量池中查找是否有对应值的字符串常量，如果没有就创建一个字符串对象放到常量池中。最终堆中创建的字符串对象的内容就引用常量池中的字符串对象的。**无论常量池中是否有，最终都要在堆中创建一个String对象且内容都引用常量池中的字符串**
...
**反射是Java被视为动态语言的关键**。有了反射可以**在运行时获取和改变代码结构**。加载完类之后，在堆内存中生成一个Class类型的对象，这个对象包含了完整的类的结构信息。通过这个Class类得到类的信息，就是反射。
