...
对于使用fork的快照持久化。fork出子进程后，子进程会共享父进程的数据段。父进程继续接收客户端的请求，当要修改数据时会将对应的数据段复制一份进行写操作，不会影响子进程的读取（读写分离）。子进程遍历但是冻结的所有数据写入rdb文件中。
...
Redis会受到客户端修改指令执行后，把指令追加到AOF日志中。（先执行后记录日志）

需要恢复内存数据时，将AOF日中的指令重做一遍。
...
为了保证在对AOF文件写入时，数据能够及时的从内存缓存中落盘。Linux提供fsync函数可以强制落盘。对应的Redis执行fsync函数就有三种不同的策略

- always：谨慎使用。每个redis写命令都要同步写入硬盘。能够将丢失的数据降到最小，但是受磁盘影响过大。
- everysec：推荐。每秒执行一次同步，将多个写命令同步到磁盘。最多丢失1s的数据。
- no：不推荐。Redis不执行fsync函数。由操作系统决定何时同步。不会对Redis性能造成影响，但是系统崩溃时会定时不定量的数据。
...
重启Redis时很少用rdb类恢复内存状态，因为会丢失大量数据。通常使用AOF日志重放，但是想使用rdb文件恢复要慢很多。

Redis4.0推出一个新的持久化选项——混合持久化。将rdb文件的内容和增量的AOF日志文件存在一起。这里的AOF不再是全量的日志，只包含持久化开始到持久化结束这段时间发生的增量AOF日志，这部分通常比较小。

在恢复内存数据时，先加载rdb文件的内容，然后追加增量AOF日志。
...
Redis对象维护了一个**24位空间，存储更新时系统的时间戳。最长存储194天的**。

新增key时会赋值一个当前系统时钟。当需要淘汰时，根据配置的策略从所有key中（或者是设置了过期时间的key中）随机选择N个，从n个键中根据时钟选出最久没有使用的一个key进行淘汰。
