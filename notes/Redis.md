...
AOF数据写入缓冲区。为了保证数据能够及时的从内存缓存中输入到磁盘中。Linux提供fsync函数可以强制落盘。对应的Redis执行fsync函数就有三种不同的策略
...
从节点是不处理过期数据的，只是被动的同步主节点的数据。主节点key到期删除后会在AOF文件中添加删除的指令。
...
为了解决这个卡顿问题，在4.0加入unlink指令，能对删除操作进行蓝处理，都给后台线程来异步回收内存。
