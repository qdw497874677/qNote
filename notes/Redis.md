...
- 自动触发
  - 根据redis.conf配置里的save m n，根据条件触发（用的是bgsave）
  - 主从复制时，主节点自动触发
...
- 持久化时，开启的子进程会拥有父进程的内存数据，父进程修改内存时，子进程不做更新了，所以**在快照持久化期间的修改不会被保存，可能造成数据丢失。**
...
**将每一个收到的写命令通过write函数追加到文件中。**将所有Redis写命令记录到日志文件中。
