...
- 静态常量池：就是class文件中的常量池，包含字符串字面量，常量（final变量，类信息，方法信息等）的符号引用
- 运行时常量池：完成类加载之后，将class文件中的静态常量池加载到内存中。
  - 每个加载后的类都在内存中有个运行时常量池
- 全局字符串常量池：生成字符串对象后，会把这个实例的引用放到字符串常量池中的hash表中驻留。一个JVM中只有一个字符串常量池
...
当方法区中类信息不可达，即类在堆中的Class对象不可达时被回收后，类才会被卸载。
...
当Class对象对应的类的对象和对应的类加载器被回收了，指自己的引用也不再引用了，Class对象就可能被回收了，回收后类信息就被卸载了。
...
- Minor GC/Young GC：Eden区被填满时，会执行Minor GC/Young GC，回收年轻代。
- Major GC/Old GC：老年代的垃圾收集叫做Major GC/Old GC。
...
通过jmap生成dump文件。format=b：表示生成二进制类型的dump文件，file=：后面写的是输出的dump文件路径，17561 ：为jvm进程id
