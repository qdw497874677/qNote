...
为了提高性能，在不改变单线程执行结果的前提下，**编译器和处理器**会对既定的代码执行顺序进行指令重排序。
...
- **NEW：新建状态。**创建一个线程实例，但还没有调用start()方法。由父线程进行空间分配，并且继承了父线程的是否Daemon、优先级、资源加载器contextClassLoader以及可继承的ThreadLocal，并且分配唯一id。
...
- **WAITING：等待状态。**表示线程进入等待状态，进入该状态表示当前线程需要等待其他线程做出一些特定动作（通知或者中断）。执行wait()、join()、park()方法进入等待状态（Lock锁中的阻塞线程用park()）。
...
- interrupt()：设置线程的中断标志位。要看线程去怎么处理中断标志。
  ...
  - 如果线程正在运行竞争synchronized、执行可重用锁的lock()，那么是不可中断的，他们会暂时忽略，直到成功获取资源后再去把中断补回来。（trylock()、lockInterruptibly()可以响应中断）
  - 判断中断标志的三种方式：
    - isInterrupted()：不会重置。
    - interrupted()：Thread的静态方法，会重置标志位。
...
下面通过stop方法或者中断线程都可以停止线程任务并完成最后的收尾
...
如果有sleep在catch中去处理中断。没有sleep可以在循环的某处通过当前线程对象的isInterrupted()来处理中断。主线程sleep10秒后对线程执行中断。
...
synchronized是Java关键字，**解决多个线程之间访问资源的同步性**，可以保证被它修饰的方法或者代码块在**同时只能有一个线程在其中**。synchronized 代码块使用一对儿 monitorenter/monitorexit 指令来获取和释放对象的Monitor。**本质就是线程对一个对象的monitor 进行获取和释放**。
...
**使用场景**：**只有一个线程进入临界区**。比较理想的情况就是，**只有一个线程在执行同步代码，一个线程消亡了之后，别的进程才来。**

1. 获取锁：当线程尝试进入同步代码时，先**检查是否为无锁状态**，如果不是，则**CAS在Mark Word中设置为偏向锁状态1**，失败就撤销并升级；如果为偏向锁状态，则**检查MW中存储的线程id**是否为自身。如果是自身则直执行同步代码；如果不是，则将MW设置为无锁，然后**用CAS设置为偏向状态，并且替换MW中的线程id**，如果替换成功则成功偏向自己，如果CAS没有成功则撤销所并升级。（**CAS设置偏向，CAS设置线程id，下次进入如果是偏向并且相同id则直接执行同步代码**）
2. 撤销锁：**出现竞争时才释放锁**。撤销要等到全局安全点。如果其他线程尝试获取锁，发现线程id和对象头中的id不一致。此时检查之前偏向的线程还是否存活，如果不存活就将锁设置为无锁状态，重新偏向新的线程。如果存活，表示对象锁被多个线程竞争，退出偏向模式，升级成轻量级锁。

##### 轻量级锁（自旋锁）

**使用场景**：多个线程交替进入临界区。这种情况下几乎没有竞争，或者有轻微的竞争。通过**短暂的忙等，去换取用户态内核态切换的开销**。加锁的响应快，但是有额外的cpu消耗。
...
2. 解锁：用CAS将锁记录中的Mark Word替换回对象头。如果失败，表示有竞争，升级成重量级锁。

尝试获取锁，自旋重试多次后（默认10次），如果都失败了，说明对于同步代码是有竞争的，就升级为重量级锁，修改锁标记位。
...
提供了类似Object的监视器方法，与Lock配合可以实现等待/通知模式。Object的监视器模型上拥有一个同步队列和一个等待队列。而基于AQS的同步器拥有一个同步队列和多个等待队列。

Condition是AQS的一个内部类。一个Condition包含一个等待队列。

使用方法的前置条件，用Lock.lock()获取锁，调用Lock.newCondition()获取Condition对象



调用Condition对象的await()，将当前线程包装成节点从尾部加入condition的等待队列（不使用CAS），然后释放锁，而其他线程调用Condition对象的signal()方法后，阻塞队列头部的线程加入同步队列，去竞争锁，从await()方法返回时，就已经获取了锁。

调用signal()方法，先检查线程是否已经获取了锁，如果获取了，将等待队列中的头结点加入到同步队列中，竞争资源。调用signalAll()方法，相当于对等待队列中的每个节点执行一次signal()方法。
...
**安全关闭**线程池。将线程池状态设置为SHUTDOWN，对线程池中没有执行任务的线程发送中断信号。正在执行的任务和队列中等待的任务完成后才彻底关闭。调用之后，如果有新任务提交，会直接根据拒绝策略拒绝。

- shutdownNow

立刻关闭。将线程池状态设置为STOP，然后给线程池中所有的线程**发送interrupt中断信号**，尝试中断，然后将任务队列中的正在等待的所有任务转移到一个List中并返回。

- isShutdown

这个方法可以返回布尔值，判断线程池**是否已经开始了关闭工作**。执行了shutdown或者shotdownNow方法后，isShutdown就返回false。

- isTerminated

这个方法可以**检测线程池是否真正终结**了，就是说线程中的所有任务是不是都执行完毕了，同时线程池已经关闭了。当所有线程都结束了，则isTerminated返回true。
...
- CPU密集型：任务需要大量的运算，没有阻塞，CPU全速运行。这种情况把线程数量保持在cpu数量就好了。**线程数不需要太多**。
- IO密集型：任务线程并不是一直在执行任务，则应该配置**尽量多的线程数**，比如CPU核数*2。
...
最好的设置几个参数的方法就是，去**监控，进入等待队列少，就减少核心数，多了就增加核心数**。
...
- 漏桶算法：限制常量流入速率，即流出速率是一个固定常量。按照固定频率从队列中取。当流入的请求数累积到漏桶容量时，则新流入的请求被拒绝。平滑流入速率。
- 令牌桶算法：固定频率往令牌桶中添加令牌，每个请求需要获取令牌。当令牌数减为零时则拒绝新的请求。限制的是平均流入速率，允许突发请求。
