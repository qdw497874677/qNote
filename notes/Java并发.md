...
不是指线程的安全，是指内存的安全。对于进程公共区域的数据，进程中所有线程都可以访问到，有可能会发生数据被意外修改。比如你获取的数据副本，和此时主内存中的**数据不一致**了。
...
加锁时会判断是否已经加了锁，如果有加锁就再判断是否获取锁的线程是否是当前线程，如果是当前线程，则给锁的获取次数加一。

释放锁时，给锁的获取次数减一，然后判断是否为0，如果为0就调用锁的唤醒方法，让锁上的阻塞的其他线程得到执行的机会。
...
AQS中维护了一个 volatile int state 和一个FIFO线程等待队列（Node节点）
...
自定义同步器一般的方式（模板方法模式的应用）：

1. 使用者继承AQS，重写指定方法，比如state的获取和释放
2. 在自定义同步器中调用AQS的模板方法，这些模板方法会一些重写的方法
...
可以安全地关闭一个线程池，调用之后不会立刻关闭，会等到线程池中正在执行的任务和队列中等待的任务完成后才彻底关闭。调用之后，如果有新任务提交，会直接根据拒绝策略拒绝。
...
这个方法可以检测线程池是否真正终结了，就是说线程中的所有任务是不是都执行完毕了，同时线程池已经关闭了。
...
立刻关闭。首先会给所有线程池中的线程发送interrupt中断信号，尝试中断，然后将任务队列中的正在等待的所有任务转移到一个List中并返回。
