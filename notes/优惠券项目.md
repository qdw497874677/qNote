...
(
	...
	constraint coupon_template_pk
		primary key (id),
	constraint coupon_template_category
		unique (category),
	constraint coupon_template_user_id
		unique (user_id)
);

create unique index coupon_template_name_uindex
	on coupon_template (name);
...
在实体类上加上注解@JsonSerialize(using = CouponTemplateSerialize.class)
...
首先自定义异步任务线程池。继承AsyncConfigurer，通过@Configuration类去注入对应的Bean。同时加上@EnableAsync注解。
...
首先定义一个微服务之间传递信息类CouponTemplateSDK。这个通用的类定义在common项目中。
