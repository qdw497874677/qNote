...
- 聚簇索引：决定数据在磁盘上的物理排序（物理连续），就是存放表数据的索引。一个表只能有一个聚簇索引，一般用主键来约束。**聚簇索引的叶子节点存储行记录。**
- 非聚簇索引：不决定数据在磁盘上的物理排序（逻辑连续）。**索引上只包含被建立索引的字段的数据**，以及一个行定位符row-locator。通过这个定位符可以找到整行数据。**非聚簇索引的叶子节点存储索引字段和主键。**根据非聚簇索引找到主键并查询主键中的行信息就是回表。
