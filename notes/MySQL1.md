...
主要作用就是**提供回滚**和**实现MVCC**（多版本并发控制）。数据修改的过程中，记录一条与当前操作相反的逻辑日志到undo log中。通过回滚日志来构建不同的视图。不同时刻启动的事务有不同的视图。当没有事务需要这些回滚日志时，会被删除。
...
3. 引擎将新数据更新到内存中的数据页中，此时这个数据页成为脏页。将旧数据记录到undo log中。同时将这个更新操作记录到redo log中（对XX表中的XX数据页的XX偏移量位置做了XX更新）。此时redo log处于prepare状态。然后通知执行器执行完成了，随时可以提交事务。Purge线程通过fsync把buffer中的数据写入磁盘。
