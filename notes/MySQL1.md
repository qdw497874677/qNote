...
- 非聚簇索引：不决定数据在磁盘上的物理排序（逻辑连续）。索**引上只包含被建立索引的数据**，以及一个行定位符row-locator。通过这个定位符可以找到整行数据。**非聚簇索引的叶子节点存储索引字段和主键。**根据非聚簇索引找到主键并查询主键中的行信息就是回表。
...
比如索引（a、b、c、d）。对于查询筛选字段为b、c、d，只有b、c用到索引。因为在前面的字段相同的情况下，后面的字段才是有序的，才能利用到索引。MySQL创建联合索引的规则是**首先对联合索引的最左边第一个字段排序，在第一个字段的排序基础上，再对第二个字段进行排序**。
