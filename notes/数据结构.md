...
- 如果元素个数大于等于阈值：就用resize()进行扩容，**重新计算hash**和下标值
- 调用createEntry()方法插入
...
- put时从先扩容再插入，改为先插入再扩容。
- put节点改为尾插
- 优化扩容过程。
  - 遍历链表用每个节点的hash和旧数组与操作（&），对于等于0的连成一个链表，等于1的连成另一个链表，把第一个链表按照之前的下标k迁移到新数组，第二个数组迁移到新数组的（k+旧数组长度）下标处。然后直接迁移到新数组中。节点的先对位置是没有变化的。
