...
> 写时复制。读写分离的思想。
>
> 写入元素的时候，先把原数组复制一份去写。写完后把原数组引用指向新数组。
>
> 写的时候是会加锁的。
...
将key通过哈希算法后定位到Segment。然后再通过一个哈希算法定位到具体元素（两次哈希方式不一样，前者对哈希值取高值，后者直接用哈希值，为了避免两次哈希值相同）。Segment中的count和hashEntry中的value，都被volatile修饰，线程拿到的总是最新值。get操作只需要读共享变量count和value，所有可以不用加锁。

put

- Segment具有锁的功能。首先定位到Segment，如果Segment没有初始化，就通过CAS赋值。然后在Segment里进行插入操作。插入需要经过两个步骤：

  - 第一步，判断是否需要对Segment里面的HashEntry数组进行扩容。
  - 第二步，通过哈希算法定位添加元素的位置，利用继承的锁的特性，将其插入指定位置（尾部）。通过tryLock()方法尝试获取锁，获取成功就插入，如果有别的线程获取锁了，就自旋调用tryLock()，超过指定次数就阻塞等待唤醒。
...
- 根据key计算哈希值。
- 如果没有初始化就先初始化。（懒汉）
- 根据key定位对应的node，如果为空也就是说没有hash冲突时，就用CAS加自旋尝试插入，成功就break。
