...
- add操作时先判断是否需要扩容，加入容量为10，当前有9个元素，一个线程正要插入，判断size不需要扩容，接下来有另外一个线程插入了，此时size已经是10了，插入数组elementDatap[10]会报错
- elementDatap[size++] i++不是原子操作，可能覆盖
...
3. 对于不为null的，计算hash值，计算下标，遍历链表，如果有**相等key**就替换value返回旧value
4. 插入之前如果需要**扩容**，就先扩容，把旧数组中的所有节点依次计算新下标头插到新数组中
...
- 计算hash值，计算数组下标，遍历链表
  - 如果有相等key的元素（先比hash后比equals）：替换value，并返回旧value
- 调用addEntry()方法插入新节点
...
- 如果元素个数大于等于阈值：就用resize()进行扩容，重新计算hash和下标值
- 调用createEntry()方法插入
...
- put节点改为尾插
- 优化扩容过程。
  - 旧数组中的一条链表的节点会有两种新下标，把他们连接成两个链表，然后直接迁移到新数组中。节点的先对位置是没有变化的。
...
3. 用Segment的put()添加元素
   1. 通过tryLock()获取到segment的锁（这个tryLock加锁失败不会阻塞），没有获取到就执行scanAndLockForPut()
   2. 计算HashEntry数组的下标
   3. 找到合适的位置插入元素
...
计算不同是为了避免两个结果关联度太高，导致同一个Segment中的元素扎堆，从而是链表过长，同时也减少了并发
...
> **在没有获取到锁的同时，去遍历链表**。如果没有相同key就创建一个新节点并停止遍历，如果有相同key就停止遍历。遍历过程中如果发现链表头结点改变，那么下次就重新开始遍历。**期间如果获取了锁或者超出最大次数直接用lock获取锁就退出**，返回node，node可能为null可能为预先创建的新节点。这个node节点在put()中判断下，如果不为空就直接头插到链表中。
...
> 创建新数组，遍历旧数组中每一个链表，先把链表中到尾部都是相同新下标的**连续子链表迁移到新数组中**，在把链表中剩下的节点从前到后**依次头插到新数组**中，最后把**新节点头插到新数组中**。期间**不需要重新哈希**。（每次迁移时新下标肯定是空的）
...
> 每次循环**累加所有Segment的modCount**，如果**两次循环的累加相等就可以返回结果**。如果循环次数超出了（执行到第四次，重试了两次都没成功），就把所有Segment**加锁**再循环，最后全部解锁。
...
3. 找到对应下标，用**synchronized** 给有节点加锁，如果是链表就按链表的方式发现相同key就替换，返回就value如果是树节点就，就按树的方式操作。
...
2. 循环：当节点插入后才break
   ...
   2. 找到key所在的下标
      ...
      2. 如果不为空，节点的hash值为否为-1：使用helpTransfer()帮助迁移数据
